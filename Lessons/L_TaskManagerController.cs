...
public class L_TaskManagerController : MonoBehaviour
{
    ...
    void Start()
    {
        Debug.Log("Initializing l_TaskManagerController...");

        // Calculate the total number of tasks.
        totalTasks = L_task1Manager.taskInfoArray.Length + 1 + L_task3Manager.L_evidences.Length; // +1 for L_task2Manager
    ...
    void UpdateTaskManagers()
    {
        ...
        L_task1Manager.gameObject.SetActive(false);
        L_task2Manager.gameObject.SetActive(false);
        L_task3Manager.gameObject.SetActive(false);

        // Activate the current task manager based on the task index.
        if (currentTaskIndex == 0 && L_task1Manager.CurrentTaskIndex < L_task1Manager.taskInfoArray.Length)
        {
            L_task1Manager.gameObject.SetActive(true);
        }
        else if (currentTaskIndex == 1 && !L_task2Manager.taskCompleted)
        {
            L_task2Manager.gameObject.SetActive(true);
        }
        else if (currentTaskIndex == 2 && !L_task3Manager.taskCompleted)
        {
            L_task3Manager.gameObject.SetActive(true);
        }
    ...
    {
        if (currentTaskIndex == 0 && taskManager == L_task1Manager.gameObject && L_task1Manager.CurrentTaskIndex < L_task1Manager.taskInfoArray.Length)
            return true;
        else if (currentTaskIndex == 1 && taskManager == L_task2Manager.gameObject && !L_task2Manager.taskCompleted)
            return true;
        else if (currentTaskIndex == 2 && taskManager == L_task3Manager.gameObject && !L_task3Manager.taskCompleted)
            return true;
        return false;
    }
    ...
    {
        L_task1Manager.UpdateHeader();
        L_task2Manager.UpdateHeader();
        L_task3Manager.UpdateHeader();
